/* ══════════════════════════════════════════
   FIREBASE DATA LAYER
   ──────────────────────────────────────────
   The Firebase web config lives in the separate file `firebase-config.js`
   (loaded BEFORE this script in index.html), which sets `window.FIREBASE_CONFIG`.
   Run setup.html to generate that file, or paste your own config into it.

   See README.md for the full first-time setup (Firebase console + hosting).

   If the config is still the 'PASTE_YOUR_CONFIG_HERE' placeholder, the page
   falls back to localStorage (single-user mode), so it still works offline.
   ══════════════════════════════════════════ */
...
const FIREBASE_DOC_PATH = ['dashboards', 'main']; // collection, doc (legacy single-doc path)
